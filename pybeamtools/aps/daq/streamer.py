...
def post_process_daq_object_via_paths(data: dict,
...
# @numba.jit('Tuple((float32[:,:], float64[:]))(float32[:,:], float64[:])', nopython=True, nogil=True,
#            parallel=True)
# def numpy_rolling_mean_manual(mat, tarr):
#     window = WINDOW
#     step = STEP
#
#     out = np.zeros(((mat.shape[0] - window) // step + 1, mat.shape[1]), dtype=mat.dtype)
...
class DAQStreamer(ABC):
    ...
    def get_latest_buffers(self, n, mode='ds'):
        ...
        matrices, times, tags = [x[0] for x in buffers], [x[1] for x in buffers], [x[2] for x in buffers]
...
class TBTStreamer(ArrayStreamer, RayMixin):
    ...
    def process_daq_object(self, x, ignore_errors=True):
        ...
        data = x.toDict()
        return post_process_daq_object_via_paths(data, self.fields_paths,
                                                 debug=self.debug,
...
class FakeTBTStreamer(TBTStreamer):
    ...
    def get_channel_settings(self, channel) -> tuple[str, dict[str, list[tuple[str, ...]]]]:
        if self.sector == 1:
            fields_paths, fstr = {'sum': [('bpm011', 'sum'), ('bpm012', 'sum')]}, 'field(bpm011.sum,bpm012.sum)'
        elif self.sector == 3:
            fields_paths, fstr = {'sum': [('bpm031', 'sum'), ('bpm032', 'sum')]}, 'field(bpm031.sum,bpm032.sum)'
        else:
            ...
            try:
                ...
                while not self.fake_source_stop:
                    ...
                    x = {name1: {'x': base_x.copy(),
                         ...
                         'time': tarr,
                         'acqClkTimestamp': {'secondsPastEpoch': int(t), 'nanoseconds': int((t - int(t)) * 1e9)}
                         }
                    pvo = FakePvObject(x)
                    fake_cnt += 1
                    # if self.debug:
