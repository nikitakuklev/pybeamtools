...
class EngineDevice:
    ...
    def write(
        self, ev: Event, value_dict: dict[str, DataT], aux_dict: dict[str, DataT]
    ) -> dict[str, DataT]:
...
class EPICSDevice(EngineDevice):
    ...
    def __init__(
        self, options: EPICSDeviceOptions, ctx: Optional[SignalContext], acc_context
    ):
    ...
    def _read_fun(self, ev, channel_name):
        ...
        if not ready:
            raise DeviceReadError(
                f"PV {self.options.pv_name} not connected"
            )  # -> {self._get_caproto_pv()}
        return self.data[channel_name]
...
class GenericDevice(EngineDevice):
    ...
    def __init__(self, options: GenericDeviceOptions, ctx: SignalContext = None):
        self.ctx = ctx
...
class ModelDevice(EngineDevice):
    ...
    def __init__(self, options: ModelDeviceOptions, ctx: SignalContext = None):
        self.data = {}
...
DEVICE_CLASS_MAP = {"echo": EchoDeviceOptions, "proxy": ProxyDeviceOptions}
