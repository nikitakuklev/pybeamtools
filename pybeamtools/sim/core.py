...
class TimedHistory:
    ...
    def to_numpy(self, include_missing: bool = False) -> np.ndarray:
        return np.vstack(
            self.times_to_numpy(include_missing), self.values_to_numpy(include_missing)
        )
    ...
    def values_to_numpy(self, include_missing: bool = False) -> np.ndarray:
        if include_missing:
            ...
            return np.array(
                [x[1].value for x in self.q if x[1].value is not None], dtype=float
            )
    ...
    def times_to_numpy(self, include_missing: bool = False) -> np.ndarray:
        if include_missing:
            ...
            return np.array(
                [x[1].time for x in self.q if x[1].value is not None], dtype=float
            )
...
class SimulationEngine:
    ...
    def subscribe_channel(self, channel_name: str) -> ChannelSubscription:
        ...
        with self.settings_lock:
            ...
            if channel_name not in self.channel_subs:
                ...
                self.channel_subs[channel_name] = ChannelSubscription(
                    channel=channel_name, engine=self
                )
        ...
        for dep in deps:
            if dep not in self.channels:
                raise SimulationError(
                    f"Unknown dependency ({dep}) for channel ({channel_name})"
                )
            if dep not in self.latest_data:
                raise DeviceDependencyError(
                    f"Missing dep ({dep}) for channel ({channel_name})"
                )
    ...
    def check_enabled_state(self, dn: str):
        ...
        if dev.state not in [DS.ENABLED, DS.ENABLED_AND_SCANNING]:
            ...
            raise DeviceDisabledError(
                f"Device ({dn}) in non-functional state ({dev.state})"
            )
    ...
    def read_fresh(
        ...
        while True:
            ...
            for i, cn in enumerate(channels):
                ...
                responses = history.ts(
                    t_start=now, t_end=None, mid_start=starting_mids[cn]
                )
    ...
    def write_channel(self, channel_name: str, value: DataT, timeout: float = 10.0):
        ...
        dt3 = t3 - t2
        self.logger.debug(
            f"{ev.uuid} | Write event ({ev.data=}) -> ({result=}) in ({dt2:.3f})|"
            f"({dt3:.3f})"
        )
        if isinstance(result.data, Exception):
            raise result.data
        ...
        while len(rb_results) < len(data_dict):
            ...
            for cn, setp in data_dict.items():
                ...
                try:
                    ...
                    d = self.read_fresh(
                        [cn_rb], timeout=0.0, now=now, **readback_kwargs
                    )
            ...
            if (timeout is not None and dt > timeout) or (
                ...
                for k, v in data_dict.items():
                    ...
                    logger.debug(
                        f"WAV timeout: {k=} -> {data_dict[k]=} vs {last_read[k]=}"
                    )
                raise WriteTimeoutError(
                    f"WAV: readback timeout {bads=} vs {bads_read=}"
                )
            time.sleep(0.1)
    ...
    def push_full_update_to_device(self, dev: EngineDevice, ignore_state: bool = False):
    ...
    def scan_until(self, threshold_time: float):
        ...
        with self.settings_lock:
            while True:
                ...
                for dn in device_names:
                    if (
                        self.scan_periods[dn] > 0
                        and self.next_scan_time[dn] <= threshold_time
                    ):
                ...
                try:
                    ev = Event(
                        op=OP.SCAN, dn=dn, txid=self.next_txid(), t_event=t_scheduled
                    )
                    dev = self.devices_map[dn]
                    try:
                        if self.TRACE:
            ...
            try:
                ...
                with self.settings_lock:
                    ...
                    if ev.op == OP.ENABLE:
                        ...
                        try:
                            ...
                            for cn, v in ev.data.items():
                                ...
                                assert cn in self.dev_channels_map[dn], (
                                    f"Channel ({cn}) not " f"registered to ({dev.name})"
                                )
                            evx = Event(
                                op=OP.UPDATE_PROP,
                                dn=dn,
                        ...
                        if self.TRACE:
                            self.logger.debug(
                                f"{ev.uuid} | Propagate ({ev.data}) at "
                                f"({t_process})"
                            )
                        ...
                        try:
                            ...
                            if any(x not in self.dev_deps_map[dn] for x in ev.data):
                                raise DeviceError(
                                    f"Invalid data ({ev.data}) for update of ({dn})"
                                )
                            for cn, v in ev.data.items():
                                self.ensure_channel_exists(cn)
                            ...
                            self.logger.warning(
                                f"{ev.uuid} | Update of disabled device ({dn})"
                            )
                            ev.cb(Result(t_process, ex, False))
                        except DeviceDependencyError as ex:
                            dev.state = DS.ERROR_DEPENDENCIES
                            self.logger.debug(
                                f"{ev.uuid} | Cannot update ({dn}) due to missing "
                                f"deps"
                            )
                            ev.cb(Result(t_process, ex, False))
                        except Exception as ex:
                        ...
                        try:
                            self.check_enabled_state(dn)
                            aux_dict = {
                                k: self.latest_data[k] for k in self.dev_deps_map[dn]
                            }
                            for cn, v in ev.data.items():
                                self.ensure_channel_exists(cn)
                            ...
                            self.logger.debug(
                                f"{ev.uuid} | Write device disabled error ({ex})"
                            )
                            self.logger.debug(f"{ev.uuid} | {traceback.format_exc()}")
                            ev.cb(Result(t_process, ex, False))
                        except Exception as ex:
                        ...
                        try:
                            ...
                            self.logger.debug(
                                f"{ev.uuid} | Read of disabled device attempted"
                            )
                            ev.cb(Result(t_process, ex, False))
                        except Exception as ex:
                        ...
                        try:
                            ...
                            self.logger.debug(
                                f"{ev.uuid} | Read now of disabled device attempted"
                            )
                            ev.cb(Result(t_process, ex, False))
                        except Exception as ex:
                            dev.state = DS.ERROR_READ
                ...
                self.update_thread_running = False
                self.logger.error(
                    f"Fatal error in update thread: {traceback.format_exc()}"
                )
    ...
    def _propagate_callbacks(self, t_run: float, dn: str, ev: Event):
        ...
        for dn in dn_list:
            ...
            related_data = {
                k: v for k, v in ev.data.items() if k in self.dev_deps_map[dn]
            }
    ...
    def _validate_update_data(self, dn, data):
        for channel_name, value in data.items():
            ...
            if channel_name not in self.dev_deps_map[dn]:
                raise SimulationError(
                    f"Channel ({channel_name}) not registered to ({dn})"
                )
