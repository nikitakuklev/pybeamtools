...
def set_and_verify_multiple(
    self,
    pvdict,
    timeout=1.0,
    readback_kwargs=None,
    ...
    for pvn, value in pvdict.items():
        ...
        assert (
            low <= value <= high
        ), f"PV {pv_input} value {value} outside {low}<->{high}"
        assert atol < np.abs(
            high - low
        ), f"PV {pv_input} tolerance {atol} larger than limit range {low}<->{high}"
    ...
    while len(values) < len(pvdict):
        ...
        for pvn, value in pvdict.items():
            ...
            if pv_read is None:
                continue
            atol = df.loc[pv_input.name, "atol"]
            val = self.read_fresh(pv_read, timeout=timeout, now=now, **readback_kwargs)[
                0
            ]
            last_read[pvn] = val
