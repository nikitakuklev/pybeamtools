...
class MockSetupPairDevice:
    ...
    def create(self, sim=None):
        ...
        for i, el in enumerate(self.variables):
            ...
            magmodel = RealisticModel(
                RealisticModelOptions(
                    name="magmodel", value=ival, noise=self.noise, **pmodel_kwargs
                ),
                sim.time(),
            )
            mag = ModelPairDevice(
        ...
        for i, c in enumerate(self.constants):
            e = EchoDevice(
                EchoDeviceOptions(
                    name=c, data={c: 5.0 + i * 10}, scan_period=self.scan_period_extra
                )
            )
    ...
    def general_objective_device(self, objective):
        ...
        gopt = GenericDeviceOptions(
            name=f"DEV:{objective}",
...
class MockGeneratorEvents:
    def __init__(
        ...
        self.extra_vars = (
            extra_vars if extra_vars is not None else []
        )  # ['ECHO:0', 'ECHO:1']
        ...
        if evaluation_functions is None:
            evaluation_functions = {
                o: Quadratic(n_var=len(variables)) for o in objectives
            }
        assert isinstance(evaluation_functions, dict)
        self.evaluation_fuctions = evaluation_functions
    ...
    def create(self, sim=None):
        ...
        for i, el in enumerate(self.variables):
            ...
            g_in = self.general_variable_input_device(
                el, mag, self.input_var_change_callback
            )
    ...
    def get_objective_value(self, objective, overrides: dict[str, float] = None):
        ...
        if objective in self.evaluation_variables:
            ...
            inputs = np.array([self.models_dict[var].read() for var in self.variables])
            if self.TRACE:
    ...
    def input_var_change_callback(self, device):
        ...
        for objective in self.objectives:
            if objective in self.evaluation_variables:
                if (
                    self.base_names[device.name]
                    not in self.evaluation_variables[objective]
                ):
                    continue
            model = self.models_dict[objective]
    ...
    def general_variable_output_device(self, cn, model: RealisticMagnet):
        ...
        gopt = GenericDeviceOptions(
            name=cn, update_fun=update_fun, read_fun=read_fun, channel_map={cn: {}}
        )
        g = GenericDevice(gopt)
        g._src = cn
        return g
    ...
    def general_objective_device(self, model):
        ...
        gopt = GenericDeviceOptions(
            name=el, update_fun=update_fun, read_fun=read_fun, channel_map={el: {}}
        )
        g = GenericDevice(gopt)
        g._src = el
        return g
