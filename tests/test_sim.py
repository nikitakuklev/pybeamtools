...
import numpy as np
import pytest
from pybeamtools.sim.core import SimulationEngine, ChannelMap, ChannelMapSet, SimulationError
...
def test_device_add(sim_engine):
    ...
    sim_engine.add_device(mag2, period=5.0)
    assert list(sim_engine.devices_list) == ['mag1', 'mag2']
    assert sim_engine.devices_list['mag1'] == mag

    def eval_mag(device, output):
        return device.read()

    mag3 = RealisticMagnet(name='mag3', value=0)
    m = ChannelMap(device=mag3, channels='MAG3:CHANNEL', read_fun=eval_mag)
    chm = ChannelMapSet(maps=[m])
    with pytest.raises(AssertionError):
        sim_engine.add_mapper(chm)

    m = ChannelMap(device=mag2, channels='MAG2:CHANNEL', read_fun=eval_mag)
    chm = ChannelMapSet(maps=[m])
    sim_engine.add_mapper(chm)
...
def test_device_set(sim_engine: SimulationEngine):
    ...
    m = ChannelMap(device=mag2, channels='MAG2:CHANNEL',
                   read_fun=read_fun, write_fun=write_fun)
    chm = ChannelMapSet(maps=[m])
    sim_engine.add_mapper(chm)

    assert sim_engine.devices_channels_map == {'mag2': ['MAG2:CHANNEL']}
