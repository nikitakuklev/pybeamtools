...
from pybeamtools.sim.pddevices import CounterDevice, CounterDeviceOptions, DS, EchoDevice, \
    EchoDeviceOptions, ModelDevice, \
    ModelDeviceOptions, ModelPairDevice, ModelPairDeviceOptions, ProxyDevice, \
...
def sim_engine_with_devices(sim_engine) -> SimulationEngine:
    ...
    proxy1 = ProxyDevice(ProxyDeviceOptions(name='proxy1', channel_map={
        'TEST:PROXY:1': {'TEST:ECHO:1': TRIG.PROPAGATE}
    }))
    proxy2 = ProxyDevice(ProxyDeviceOptions(name='proxy2', channel_map={
        'TEST:PROXY:2': {'TEST:ECHO:2': TRIG.IGNORE}
    }))
    proxy3 = ProxyDevice(ProxyDeviceOptions(name='proxy3', channel_map={
        'TEST:PROXY:3': {'TEST:PROXY:1': TRIG.PROPAGATE}
    }))
    proxy4 = ProxyDevice(ProxyDeviceOptions(name='proxy4', channel_map={
...
def test_device_add(sim_engine):
    ...
    echo1 = EchoDevice(EchoDeviceOptions(name='echo1', data={'TEST:ECHO:1': 5.0}))
    echo1dup = EchoDevice(EchoDeviceOptions(name='echo1', data={'TEST:ECHO:1': 5.0}))
    echo2 = EchoDevice(EchoDeviceOptions(name='echo2', data={'TEST:ECHO:2': 15.0}))
...
def test_adv_write(sim_engine: SimulationEngine):
    ...
    sim_engine.time_fun = local_time
    ctx = SignalContext(se=sim_engine)
    echo1 = EchoDevice(EchoDeviceOptions(name='echo1',
...
def test_modelpairdevice(sim_engine: SimulationEngine):
    ...
    mag1model = RealisticModel(RealisticModelOptions(name='mag1model', value=0.5), t)
    mag1 = ModelPairDevice(ModelPairDeviceOptions(name='mag1',
                                                  readback_name='mag1rb', device=mag1model))
    ctx.add_device(echo1)
    ctx.add_device(mag1)
...
def test_modelpair_scan(sim_engine: SimulationEngine):
    ...
    t_local = 5.0
    sim_engine.scan_until(t_local)
    assert mag1.stats == {'update': 1, 'read': 0, 'write': 0, 'scan': 10}
    assert echo1.stats == {'update': 1, 'read': 0, 'write': 0, 'scan': 10}
    ...
    assert sim_engine.latest_data['MAG1RB'] == 1.5
    assert sim_engine.read_channel('MAG1RB') == 1.5
    assert mag1.stats == {'update': 1, 'read': 4, 'write': 1, 'scan': 10}
    assert echo1.stats == {'update': 1, 'read': 0, 'write': 0, 'scan': 10}

    t_local = 15.0
    sim_engine.scan_until(t_local)
    assert mag1.stats == {'update': 1, 'read': 4, 'write': 1, 'scan': 30}
...
def test_device_deps(sim_engine: SimulationEngine):
    ...
    assert proxy1.stats == {'update': 0, 'read': 0, 'write': 0, 'scan': 0}
    assert proxy2.stats == {'update': 0, 'read': 0, 'write': 0, 'scan': 0}
    ...
    sim_engine.process_events()
    assert echo1.stats == {'update': 1, 'read': 0, 'write': 2, 'scan': 0}
